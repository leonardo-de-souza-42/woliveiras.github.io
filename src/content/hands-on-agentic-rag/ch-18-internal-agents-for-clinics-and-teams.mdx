...
VetSupport already filters retrieval by pet, which is the foundation this chapter builds on. The role-based access described here is a design extension, not yet implemented in the harness. It works the same way: the retrieval query would carry the requester's role and scope, and exclude anything outside it before ranking. A document the role may not see is never retrieved, never ranked, and never placed in context. It does not exist as far as that request is concerned.
...
- Retrieval should filter by role and scope, not only by pet (the harness filters by pet today).
